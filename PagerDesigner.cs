...
namespace Wuqi.Webdiyer
{
    ...
    public class AspNetPagerActionList : DesignerActionList
    {
        ...
        public override DesignerActionItemCollection GetSortedActionItems()
        {
            ...
            actionItems.Add(new DesignerActionHeaderItem("分页"));
            actionItems.Add(new DesignerActionPropertyItem("UrlPaging", "启用Url分页", "分页"));
            actionItems.Add(new DesignerActionMethodItem(this, "ShowUrlConfigForm", "Url分页设置", "分页", true));
            ...
            actionItems.Add(new DesignerActionPropertyItem("ShowMoreButtons", "显示更多页(...)按钮", "外观"));
            actionItems.Add(new DesignerActionPropertyItem("CurrentPageButtonPosition", "当前页数字索引按钮位置", "外观"));
            actionItems.Add(new DesignerActionMethodItem(this,"SetLayoutType", "控件及分页导航元素布局方式...", "外观"));
        ...
        {
            get { return pager.ShowMoreButtons; }
            set { SetProperty("ShowMoreButtons", value); }
        }
        ...
        {
            get { return pager.ShowPrevNext; }
            set { SetProperty("ShowPrevNext", value); }
        }

        public bool ReverseUrlPageIndex
        {
            get { return pager.ReverseUrlPageIndex; }
            set { SetProperty("ReverseUrlPageIndex", value); }
        }

        public void SetLayoutType()
        {
            LayoutForm f = new LayoutForm(pager.LayoutType, pager.PagingButtonLayoutType);
            if(f.ShowDialog()==System.Windows.Forms.DialogResult.OK)
            {
                SetProperty("LayoutType", f.LayoutType);
                SetProperty("PagingButtonLayoutType", f.PagingButtonLayoutType);
            }
        }

        public PagingButtonPosition CurrentPageButtonPosition
        {
            get { return pager.CurrentPageButtonPosition; }
            set { SetProperty("CurrentPageButtonPosition", value); }
        }
        ...
        public void ShowUrlConfigForm()
        {
            UrlPagingForm f = new UrlPagingForm(pager.UrlPageIndexName,pager.UrlPaging,pager.ReverseUrlPageIndex,pager.EnableUrlRewriting,pager.UrlRewritePattern);
            if (f.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                SetProperty("UrlPaging", f.UrlPaging);
                if (!string.IsNullOrEmpty(f.UrlPageIndexName))
                    SetProperty("UrlPageIndexName", f.UrlPageIndexName);
                SetProperty("ReverseUrlPageIndex", f.ReverseUrlPageIndex);
                SetProperty("EnableUrlRewriting", f.EnableUrlRewriting);
                if (!string.IsNullOrEmpty(f.UrlRewritePattern))
                    SetProperty("UrlRewritePattern", f.UrlRewritePattern);
            }
        }
